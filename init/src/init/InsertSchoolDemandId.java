		...
		try(Connection conn = DriverManager.getConnection(URL,USERNAME,PASSWORD);
				PreparedStatement pstmt = conn.prepareStatement(INSERT);)
			{
				...
				try
				{
					...
					pstmt.setInt(2,12);  	                    // participant 預計參與的學生人數
...
//					pstmt.setTimestamp(8,new java.sql.Timestamp(System.currentTimeMillis()));	// createDate 建立日期(計畫需求發佈的日期)- 可以算出媒合期截止時間
				...
				try
				{
					...
					pstmt.setInt(2,18);  	                    // participant 預計參與的學生人數
					...
					pstmt.setString(5,"教育資源匱乏的學童們");	// activitySuitable 參與本活動的學生狀態
					pstmt.setString(6,"張凱文");		            // activityHost 活動負責人(學校方負責此項計畫的聯絡人)
...
//					pstmt.setTimestamp(8,new java.sql.Timestamp(System.currentTimeMillis()));	// createDate 建立日期(計畫需求發佈的日期)- 可以算出媒合期截止時間
				...
				try
				{
					...
					pstmt.setString(6,"林凱柔");		            // activityHost 活動負責人(學校方負責此項計畫的聯絡人)
					pstmt.setString(7,"0952-388-970");		    // activityContact 負責人聯絡方式(市話、手機皆可，前端提醒使用者依照輸入的格式)
//					pstmt.setTimestamp(8,new java.sql.Timestamp(System.currentTimeMillis()));	// createDate 建立日期(計畫需求發佈的日期)- 可以算出媒合期截止時間
					SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
					pstmt.setTimestamp(8,new java.sql.Timestamp(sdf.parse("2015-05-100").getTime()));
				...
				try
				{
					...
					pstmt.setInt(2,7);  	                    // participant 預計參與的學生人數
					...
					pstmt.setString(6,"林家均");		            // activityHost 活動負責人(學校方負責此項計畫的聯絡人)
...
//					pstmt.setTimestamp(8,new java.sql.Timestamp(System.currentTimeMillis()));	// createDate 建立日期(計畫需求發佈的日期)- 可以算出媒合期截止時間
				...
				try
				{
					...
					pstmt.setInt(2,13);  	                    // participant 預計參與的學生人數
					...
					pstmt.setString(6,"黃德文");		            // activityHost 活動負責人(學校方負責此項計畫的聯絡人)
					pstmt.setString(7,"0939-370-492");		    // activityContact 負責人聯絡方式(市話、手機皆可，前端提醒使用者依照輸入的格式)
//					pstmt.setTimestamp(8,new java.sql.Timestamp(System.currentTimeMillis()));	// createDate 建立日期(計畫需求發佈的日期)- 可以算出媒合期截止時間
					SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
					pstmt.setTimestamp(8,new java.sql.Timestamp(sdf.parse("2015-09-23").getTime()));
					...
					pstmt.setString(10,"洽談中");	            //demandStatus  計畫狀態(待洽談、洽談中、待洽談、洽談失敗)
				...
				try
				{
					...
					pstmt.setString(6,"李綜民");		            // activityHost 活動負責人(學校方負責此項計畫的聯絡人)
...
//					pstmt.setTimestamp(8,new java.sql.Timestamp(System.currentTimeMillis()));	// createDate 建立日期(計畫需求發佈的日期)- 可以算出媒合期截止時間
				...
				try
				{
					...
					pstmt.setString(6,"鐘家浩");		            // activityHost 活動負責人(學校方負責此項計畫的聯絡人)
					pstmt.setString(7,"0973-248-832");		    // activityContact 負責人聯絡方式(市話、手機皆可，前端提醒使用者依照輸入的格式)
//					pstmt.setTimestamp(8,new java.sql.Timestamp(System.currentTimeMillis()));	// createDate 建立日期(計畫需求發佈的日期)- 可以算出媒合期截止時間
					SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
					pstmt.setTimestamp(8,new java.sql.Timestamp(sdf.parse("2015-09-25").getTime()));
				...
				try
				{
					...
					pstmt.setInt(2,15);  	                    // participant 預計參與的學生人數
					...
					pstmt.setString(6,"黃偉翰");		            // activityHost 活動負責人(學校方負責此項計畫的聯絡人)
