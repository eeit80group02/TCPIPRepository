...
public class MissionDAOJdbc implements MissionDAO {
	...
	private static final String FIND_BY_MISSIONSETID = "SELECT missionId,missionSetId,name,host,endTime,missionPriority,missionPosition,missionStatus,mainMissionId FROM Mission WHERE missionSetId = ?";

	@Override
	public List<MissionBean> findByMissionSetId(int id) {
		List<MissionBean> result = new ArrayList<MissionBean>();
		MissionBean bean = null;
		try (Connection conn = datasource.getConnection();
				PreparedStatement pstmt = conn.prepareStatement(FIND_BY_MISSIONSETID);) {
			pstmt.setInt(1, id);
			try (ResultSet rs = pstmt.executeQuery();) {
				while (rs.next()) {
					bean = new MissionBean();
	...
	private static final String FIND_BY_MAINMISSIONID = "SELECT missionId,missionSetId,name,host,endTime,missionPriority,missionPosition,missionStatus,mainMissionId FROM Mission WHERE mainMissionId = ?";

	@Override
	public List<MissionBean> findByMainMissionId(int id) {
		List<MissionBean> result = new ArrayList<MissionBean>();
		MissionBean bean = null;
		try (Connection conn = datasource.getConnection();
				PreparedStatement pstmt = conn.prepareStatement(FIND_BY_MAINMISSIONID);) {
			pstmt.setInt(1, id);
			try (ResultSet rs = pstmt.executeQuery();) {
				while (rs.next()) {
					bean = new MissionBean();
