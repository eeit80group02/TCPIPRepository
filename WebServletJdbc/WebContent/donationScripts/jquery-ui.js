		...
		(function($) {
			...
			var mouse = $.widget("ui.mouse", {
				...
				_mouseMove : function(event) {
					// Only check for mouseups outside the document if you've
					// moved inside the document
					...
					if (this._mouseMoved) {
						...
						if ($.ui.ie && (!document.documentMode || document.documentMode < 9) && !event.button) {
							...
							// Iframe mouseup check - mouseup occurred in
							// another document
						} else if (!event.which) {
							return this._mouseUp(event);
						}
			...
			(function() {
				...
				$.ui.position = {
					...
					flip : {
						...
						top : function(position, data) {
							var within = data.within, withinOffset = within.offset.top + within.scrollTop, outerHeight = within.height, offsetTop = within.isWindow ? within.scrollTop : within.offset.top, collisionPosTop = position.top - data.collisionPosition.marginTop, overTop = collisionPosTop - offsetTop, overBottom = collisionPosTop + data.collisionHeight - outerHeight - offsetTop, top = data.my[1] === "top", myOffset = top ? -data.elemHeight
			...
			var accordion = $.widget("ui.accordion", {
				...
				_setOption : function(key, value) {
					...
					// setting collapsible: false while collapsed; open first
					// panel
					...
					// #5332 - opacity doesn't cascade to positioned elements in
					// IE
				...
				_activate : function(index) {
					...
					// trying to collapse, simulate a click on the currently
					// active header
				...
				_toggle : function(data) {
					...
					// handle activating a panel during the animation for
					// another activation
					...
					// if we're collapsing, then keep the collapsing header in
					// the tab order
				...
				_animate : function(toShow, toHide, data) {
					...
					// fall back from options to animation in case of partial
					// down settings
				...
				_create : function() {
					...
					this._on(this.menu.element, {
						mousedown : function(event) {
							...
							// clicking on the scrollbar causes focus to shift
							// to the body
							// but we can't detect a mouseup or a click
							// immediately afterward
							// so we have to track the next mousedown and close
							// the menu if
			...
			function Datepicker() {
				...
				this.regional[""] = { // Default regional settings
					...
					currentText : "Today", // Display text for current month
											// link
					monthNames : [ "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December" ], // Names
																																								// of
																																								// months
																																								// for
																																								// drop-down
					...
					.extend(
							Datepicker.prototype,
							{
								...
								_checkExternalClick : function(event) {
									...
									if ((($target[0].id !== $.datepicker._mainDivId && $target.parents("#" + $.datepicker._mainDivId).length === 0 && !$target.hasClass($.datepicker.markerClassName) && !$target.closest("." + $.datepicker._triggerClass).length && $.datepicker._datepickerShowing && !($.datepicker._inDialog && $.blockUI)))
								...
								parseDate : function(format, value, settings) {
									...
									var iFormat, dim, extra, iValue = 0, shortYearCutoffTemp = (settings ? settings.shortYearCutoff : null) || this._defaults.shortYearCutoff, shortYearCutoff = (typeof shortYearCutoffTemp !== "string" ? shortYearCutoffTemp : new Date().getFullYear() % 100 + parseInt(shortYearCutoffTemp, 10)), dayNamesShort = (settings ? settings.dayNamesShort : null)
											|| this._defaults.dayNamesShort, dayNames = (settings ? settings.dayNames : null) || this._defaults.dayNames, monthNamesShort = (settings ? settings.monthNamesShort : null) || this._defaults.monthNamesShort, monthNames = (settings ? settings.monthNames : null) || this._defaults.monthNames, year = -1, month = -1, day = -1, doy = -1, literal = false, date,
								...
								_generateHTML : function(inst) {
									var maxDraw, prevText, prev, nextText, next, currentText, gotoDate, controls, buttonPanel, firstDay, showWeek, dayNames, dayNamesMin, monthNames, monthNamesShort, beforeShowDay, showOtherMonths, selectOtherMonths, defaultDate, html, dow, row, group, col, selectedDate, cornerClass, calender, thead, day, daysInMonth, leadDays, curRows, numRows, printDate, dRow, tbody, daySettings, otherMonth, unselectable, tempDate = new Date(), today = this
											._daylightSavingAdjust(new Date(tempDate.getFullYear(), tempDate.getMonth(), tempDate.getDate())), // clear
																																				// time
									isRTL = this._get(inst, "isRTL"), showButtonPanel = this._get(inst, "showButtonPanel"), hideIfNoPrevNext = this._get(inst, "hideIfNoPrevNext"), navigationAsDateFormat = this._get(inst, "navigationAsDateFormat"), numMonths = this._getNumberOfMonths(inst), showCurrentAtPos = this._get(inst, "showCurrentAtPos"), stepMonths = this._get(inst, "stepMonths"), isMultiMonth = (numMonths[0] !== 1 || numMonths[1] !== 1), currentDate = this
											._daylightSavingAdjust((!inst.currentDay ? new Date(9999, 9, 9) : new Date(inst.currentYear, inst.currentMonth, inst.currentDay))), minDate = this._getMinMaxDate(inst, "min"), maxDate = this._getMinMaxDate(inst, "max"), drawMonth = inst.drawMonth - showCurrentAtPos, drawYear = inst.drawYear;
									...
									prev = (this._canAdjustMonth(inst, -1, drawYear, drawMonth) ? "<a class='ui-datepicker-prev ui-corner-all' data-handler='prev' data-event='click'" + " title='" + prevText + "'><span class='ui-icon ui-icon-circle-triangle-" + (isRTL ? "e" : "w") + "'>" + prevText + "</span></a>" : (hideIfNoPrevNext ? ""
									...
									next = (this._canAdjustMonth(inst, +1, drawYear, drawMonth) ? "<a class='ui-datepicker-next ui-corner-all' data-handler='next' data-event='click'" + " title='" + nextText + "'><span class='ui-icon ui-icon-circle-triangle-" + (isRTL ? "w" : "e") + "'>" + nextText + "</span></a>" : (hideIfNoPrevNext ? ""
									...
									buttonPanel = (showButtonPanel) ? "<div class='ui-datepicker-buttonpane ui-widget-content'>" + (isRTL ? controls : "") + (this._isInRange(inst, gotoDate) ? "<button type='button' class='ui-datepicker-current ui-state-default ui-priority-secondary ui-corner-all' data-handler='today' data-event='click'" + ">" + currentText + "</button>" : "")
											+ (isRTL ? "" : controls) + "</div>" : "";
									...
									for (row = 0; row < numMonths[0]; row++) {
										...
										for (col = 0; col < numMonths[1]; col++) {
											...
											calender += "<div class='ui-datepicker-header ui-widget-header ui-helper-clearfix" + cornerClass + "'>" + (/all|left/.test(cornerClass) && row === 0 ? (isRTL ? next : prev) : "") + (/all|right/.test(cornerClass) && row === 0 ? (isRTL ? prev : next) : "")
													+ this._generateMonthYearHeader(inst, drawMonth, drawYear, minDate, maxDate, row > 0 || col > 0, monthNames, monthNamesShort) + // draw
																																													// month
																																													// headers
			...
			$.widget("ui.draggable", $.ui.mouse, {
				...
				_blurActiveElement : function(event) {
					...
					try {
						...
						// Support: IE9, IE10
						// If the <body> is blurred, IE will switch windows, see
						// #9520
						if (document.activeElement && document.activeElement.nodeName.toLowerCase() !== "body") {

							// Blur any element that currently has focus, see
							// #4261
				...
				_mouseStart : function(event) {
					...
					/*
					 * - Position generation - This block generates everything
					 * position related - it's the core of draggables.
					 */
					...
					// as this prevents resizing of elements with right/bottom
					// set (see #7772)
				...
				_mouseStop : function(event) {
					...
					// If we are using droppables, inform the manager about the
					// drop
				...
				_mouseUp : function(event) {
					...
					// If the ddmanager is used for droppables, inform the
					// manager that dragging has stopped (see #5003)
					if ($.ui.ddmanager) {
				...
				_getParentOffset : function() {
					...
					// position is calculated based on the next positioned
					// parent
					// 2. The actual offset parent is a child of the scroll
					// parent, and the scroll parent isn't the document, which
					// means that
				...
				_setContainment : function() {
					...
					if (o.containment === "window") {
						this.containment = [ $(window).scrollLeft() - this.offset.relative.left - this.offset.parent.left, $(window).scrollTop() - this.offset.relative.top - this.offset.parent.top, $(window).scrollLeft() + $(window).width() - this.helperProportions.width - this.margins.left,
					...
					this.containment = [ (parseInt(c.css("borderLeftWidth"), 10) || 0) + (parseInt(c.css("paddingLeft"), 10) || 0), (parseInt(c.css("borderTopWidth"), 10) || 0) + (parseInt(c.css("paddingTop"), 10) || 0),
				...
				_generatePosition : function(event, constrainPosition) {
					...
					/*
					 * - Position constraining - Constrain the position to a mix
					 * of grid, containment.
					 */
					...
					if (constrainPosition) {
						...
						if (o.grid) {
							...
							// divide by 0 error causing invalid argument errors
				...
				start : function(event, ui, draggable) {
					...
					$(draggable.options.connectToSortable).each(function() {
						...
						if (sortable && !sortable.options.disabled) {
							...
							// with any changes that might have happened on the
							// page since initialization.
				...
				stop : function(event, ui, draggable) {
					...
					$.each(draggable.sortables, function() {
						...
						if (sortable.isOver) {
							...
							// sortable,
							// as this also handles revert (#9675) since the
							// draggable
							...
							// Once drag has ended, the sortable should return
							// to using
							// its original helper, not the shared helper from
							// draggable
							sortable.options.helper = sortable.options._helper;
						} else {
							// Prevent this Sortable from removing the helper.
							// However, don't set the draggable to remove the
							// helper
				...
				drag : function(event, ui, draggable) {
					$.each(draggable.sortables, function() {
						...
						if (innermostIntersecting) {
							// If it intersects, we use a little isOver variable
							// and set it once,
							// so that the move-in stuff gets fired only once.
							if (!sortable.isOver) {
								sortable.isOver = 1;

								// Store draggable's parent in case we need to
								// reappend to it later.
								...
								// Fire the start events of the sortable with
								// our passed browser event,
								// and our own helper (so it doesn't create a
								// new one)
								...
								// appended portlet,
								// modify necessary variables to reflect the
								// changes
								...
								// Inform draggable that the helper is in a
								// valid drop zone,
								// used solely in the revert option to handle
								// "valid/invalid".
								draggable.dropped = sortable.element;

								// Need to refreshPositions of all sortables in
								// the case that
								// adding to one sortable changes the location
								// of the other sortables (#9675)
								$.each(draggable.sortables, function() {
							...
							if (sortable.currentItem) {
								...
								// Copy the sortable's position because the
								// draggable's can potentially reflect
							...
							// intersected before,
							// we fake the drag stop of the sortable, but make
							...
							if (sortable.isOver) {
								...
								// Calling sortable's mouseStop would trigger a
								// revert,
								// so revert must be temporarily false until
								// after mouseStop is called.
								...
								// when the draggable entered the sortable area
								// (#9481)
								...
								// Restore and recalculate the draggable's
								// offset considering the sortable
								...
								// Inform draggable that the helper is no longer
								// in a valid drop zone
								draggable.dropped = false;

								// Need to refreshPositions of all sortables
								// just in case removing
								// from one sortable changes the location of
					...
					.widget("ui.resizable", $.ui.mouse,
							{
								...
								_respectSize : function(data) {
									...
									var o = this._vBoundaries, a = this.axis, ismaxw = this._isNumber(data.width) && o.maxWidth && (o.maxWidth < data.width), ismaxh = this._isNumber(data.height) && o.maxHeight && (o.maxHeight < data.height), isminw = this._isNumber(data.width) && o.minWidth && (o.minWidth > data.width), isminh = this._isNumber(data.height) && o.minHeight
											&& (o.minHeight > data.height), dw = this.originalPosition.left + this.originalSize.width, dh = this.position.top + this.size.height, cw = /sw|nw|w/.test(a), ch = /nw|ne|n/.test(a);
									if (isminw) {
			...
			$.ui.plugin.add("resizable", "grid", {
				...
				resize : function() {
					var outerDimensions, that = $(this).resizable("instance"), o = that.options, cs = that.size, os = that.originalSize, op = that.originalPosition, a = that.axis, grid = typeof o.grid === "number" ? [ o.grid, o.grid ] : o.grid, gridX = (grid[0] || 1), gridY = (grid[1] || 1), ox = Math.round((cs.width - os.width) / gridX) * gridX, oy = Math.round((cs.height - os.height) / gridY)
							* gridY, newWidth = os.width + ox, newHeight = os.height + oy, isMaxWidth = o.maxWidth && (o.maxWidth < newWidth), isMaxHeight = o.maxHeight && (o.maxHeight < newHeight), isMinWidth = o.minWidth && (o.minWidth > newWidth), isMinHeight = o.minHeight && (o.minHeight > newHeight);
			...
			var dialog = $.widget("ui.dialog", {
				...
				_createWrapper : function() {
					...
					// We assume that any existing aria-describedby attribute
					// means
			...
			(function(jQuery, undefined) {
				...
				color.fn = jQuery.extend(color.prototype, {
					parse : function(red, green, blue, alpha) {
						...
						// more than 1 argument specified - assume ( red, green,
						// blue, alpha )
						if (green !== undefined) {
							...
							if (red instanceof color) {
								...
								each(spaces, function(spaceName, space) {
									...
									each(space.props, function(key, prop) {
										...
										// if the cache doesn't exist, and we
										// know how to convert
										...
										// this is the only case where we allow
										// nulls for ALL properties.
			...
			var effectBlind = $.effects.effect.blind = function(o, done) {
				// Create element
				var el = $(this), rvertical = /up|down|vertical/, rpositivemotion = /up|left|vertical|horizontal/, props = [ "position", "top", "bottom", "left", "right", "height", "width" ], mode = $.effects.setMode(el, o.mode || "hide"), direction = o.direction || "up", vertical = rvertical.test(direction), ref = vertical ? "height" : "width", ref2 = vertical ? "top" : "left", motion = rpositivemotion
						.test(direction), animation = {}, show = mode === "show", wrapper, distance, margin;
				...
				el.animate(el.to, {
					...
					complete : function() {
						...
						if (!restore) {
							...
							// we need to calculate our new positioning based on
							// the scaling
							if (position === "static") {
								el.css({
									position : "relative",
			...
			var progressbar = $.widget("ui.progressbar", {
				...
				_setOptions : function(options) {
					// Ensure "value" option is set after other values (like
					// max)
			...
			var selectmenu = $.widget("ui.selectmenu", {
				...
				_drawMenu : function() {
					...
					this.menuInstance = this.menu.menu({
						...
						focus : function(event, ui) {
							...
							// Prevent inital focus from firing and check if its
							// a newly focused item
					...
					// Selects often contain empty items, but never contain
					// dividers
				...
				open : function(event) {
					...
					// If this is the first time the menu is being opened,
					// render the items
					if (!this.menuItems) {
						this._refreshMenu();
					} else {
				...
				_setSelection : function() {
					...
					// support: IE
					// Setting the text selection kills the button focus in IE,
					// but
			...
			var slider = $.widget("ui.slider", $.ui.mouse, {
				...
				// (how many times can you page up/down to go through the whole
				// range)
				numPages : 5,
				...
				// _value() returns value trimmed by min and max, aligned by
				// step
				_value : function() {
				...
				// returns the step-aligned value that val is closest to,
			...
			var sortable = $.widget("ui.sortable", $.ui.mouse,
					{
						...
						_setContainment : function() {
							...
							if (o.containment === "document" || o.containment === "window") {
								this.containment = [ 0 - this.offset.relative.left - this.offset.parent.left, 0 - this.offset.relative.top - this.offset.parent.top, o.containment === "document" ? this.document.width() : this.window.width() - this.helperProportions.width - this.margins.left,
							...
							if (!(/^(document|window|parent)$/).test(o.containment)) {
								...
								this.containment = [ co.left + (parseInt($(ce).css("borderLeftWidth"), 10) || 0) + (parseInt($(ce).css("paddingLeft"), 10) || 0) - this.margins.left, co.top + (parseInt($(ce).css("borderTopWidth"), 10) || 0) + (parseInt($(ce).css("paddingTop"), 10) || 0) - this.margins.top,
			...
			var spinner = $.widget("ui.spinner", {
				...
				_events : {
					...
					"mousedown .ui-spinner-button" : function(event) {
						...
						// We never want the buttons to have focus; whenever the
						// user is
						...
						function checkFocus() {
							...
							if (!isActive) {
								...
								this.previous = previous;
								// support: IE
								// IE sets focus asynchronously, so we need to
								// check if focus
								// moved off of the input because the user
					...
					// shouldn't we just stop the repeater and wait until
					// mouseup before
			...
			var tabs = $.widget("ui.tabs", {
				...
				_create : function() {
					...
					// check for length avoids error when initializing empty
					// list
				...
				_tabKeydown : function(event) {
					...
					// Navigating with control/command key will prevent
					// automatic activation
					if (!event.ctrlKey && !event.metaKey) {

						// Update aria-selected immediately so that AT think the
				...
				// Alt+page up/down moves focus to the previous/next tab (and
				// activates)
				...
				_toggle : function(event, eventData) {
					...
					// If we're collapsing, then keep the collapsing tab in the
					// tab order.
					if (toShow.length && toHide.length) {
				...
				_activate : function(index) {
					...
					// trying to collapse, simulate a click on the current
					// active header
					if (!active.length) {
						active = this.active;
					}
				...
				_getIndex : function(index) {
					// meta-function to give users option to provide a href
					// string instead of a numerical index.
			...
			var tooltip = $.widget("ui.tooltip", {
				...
				options : {
					...
					hide : true,
					// Disabled elements have inconsistent behavior across
					// browsers (#8661)
				...
				_create : function() {
					...
					// Append the aria-live region so tooltips announce
					// correctly
				...
				open : function(event) {
					...
					// No element to show a tooltip for or the tooltip is
					// already open
				...
				_updateContent : function(target, event) {
					...
					content = contentOption.call(target[0], function(response) {
						...
						that._delay(function() {
							...
							// jQuery creates a special event for focusin when
							// it doesn't
				...
				_open : function(event, target, content) {
					...
					// Content can be updated multiple times. If the tooltip
					// already
					...
					// if we have a title, clear it to prevent the native
					// tooltip
					// we have to check first to avoid defining a title if none
					// exists
					// (we don't want to cause an element to start matching
					// [title])
					//
					// We use removeAttr only for key events, to allow IE to
					// export the correct
					// accessible attributes. For mouse events, set to empty
					// string to avoid
					// native tooltip showing up (happens only when removing
					// inside mouseover).
					if (target.is("[title]")) {
						if (event && event.type === "mouseover") {
							target.attr("title", "");
				...
				_registerCloseHandlers : function(event, target) {
					...
					// Only bind remove handler for delegated targets.
					// Non-delegated
				...
				close : function(event) {
					...
					if (!tooltipData) {
						...
						// additional data once there's actually content to show
						// (in _open()). So even if the
					...
					// only set title if we had one before (see comment in
					// _open())
				...
				_destroy : function() {
					...
					$.each(this.tooltips, function(id, tooltipData) {
						...
						// Restore the title
						if (element.data("ui-tooltip-title")) {
							// If the title attribute has changed since open(),
							// don't restore
